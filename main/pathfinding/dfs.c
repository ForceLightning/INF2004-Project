...
dfs_is_all_reachable_visited (grid_t *p_grid, navigator_state_t *p_navigator)
{
    ...
    // Step 2: Add the current node to the reachable set.
    //
    grid_cell_t *p_next_node = &p_navigator->p_current_node;
    insert(&reachable_set, p_next_node, p_next_node->h);
                     ...
                     navigator_state_t *p_navigator)
{
    ...
    while (0 < open_set.size)
    {
        ...
        for (uint8_t neighbour_idx = 0; 4 > neighbour_idx; neighbour_idx++)
        {
            // Step 2: Ensure that the neighbour is not null.
            //
            grid_cell_t *p_neighbour
                = p_current_node.p_maze_node->p_next[neighbour_idx];
            if (NULL == p_neighbour)
            {
                continue;
            }
            ...
            // Step 3: Ensure that the neighbour is not in the reachable set.
            ...
            // Step 4: Ensure that the neighbour is not in the open set, then
